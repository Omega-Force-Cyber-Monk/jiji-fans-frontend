  ...
  return (
    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 max-w-[1440px] mx-auto px-4 sm:px-10 lg:px-16">
      ...
      <div className="flex-1 flex flex-col md:flex-row justify-between items-center sm:items-start md:items-start mt-4 sm:mt-0 pt-0 sm:pt-4 gap-6 w-full">
        ...
        {/* Right side container: CTA buttons & Copy Link */}
        {((isUser && !isSubscribed) || isNotOwner || isAdmin || (isOwner && vanitySlug)) && (
          <div className="flex flex-col sm:flex-row items-center justify-center md:justify-end gap-3 shrink-0 mt-2 md:mt-4">
            {/* Vanity URL (owner only) */}
            {isOwner && vanitySlug && (
              <div className="flex items-center gap-2 text-[11px] text-secondary-text bg-secondary-bg border border-border-primary rounded-xl px-4 py-2 w-fit">
                <span className="font-medium truncate opacity-70">
                  {origin ? `${origin}/${vanitySlug}` : `/${vanitySlug}`}
                </span>
                <button
                  onClick={onCopyLink}
                  className="text-brand-primary font-semibold hover:underline bg-transparent border-none cursor-pointer"
                >
                  Copy
                </button>
              </div>
